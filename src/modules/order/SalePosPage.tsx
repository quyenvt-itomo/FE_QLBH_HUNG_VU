...
const typeOf = (mode: Mode): OrderType =>
  mode === "sale" ? OrderType.SALE : OrderType.SALE_RETURN;

/** Full-screen POS shell. The working list is persisted in the order cache. */
const SalePosPage: React.FC = () => {
  const dispatch = useDispatch();
  const location = useLocation();
  const [mode, setMode] = React.useState<Mode>(location.pathname.includes("sales-returns") ? "saleReturn" : "sale");
  ...
  useEffect(() => {
    orders.forEach((order) =>
      dispatch(addOrderCache({ id: order.id, order: { ...order, type: typeOf(mode) } })),
    );
  }, [dispatch, mode, orders]);

  const cachedOrders = useMemo(
    () => Object.values(cached).filter((order) => order.type === typeOf(mode)),
    [cached, mode],
  );
    ...
    <div className="fixed inset-0 z-40 flex flex-col overflow-hidden bg-slate-100 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Segmented
          ...
          options={[
            ...
            { label: "Đổi trả", value: "saleReturn" },
          ]}
        />
      </div>
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Card
          ...
          title={<span>Đơn {mode === "sale" ? "bán" : "đổi trả"}</span>}
