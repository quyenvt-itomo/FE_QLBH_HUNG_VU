...
export const purchaseLineToExcelRow = (line: any, hidePrice = false) => {
  ...
  return [
    ...
    hidePrice ? "" : price,
    hidePrice ? "" : discount,
    hidePrice ? "" : line.discountType || "đ",
    quantity,
    hidePrice ? "" : quantity * price - discount,
  ];
};
...
export class PurchaseFile {
  static async exportExcel(purchase: Purchase, options: PurchaseFileOptions = {}) {
    ...
    setColumns(sheet);
    styleHeader(sheet);
    sheet.addRows((purchase.lines || []).map((line) => purchaseLineToExcelRow(line, options.hidePrice)));
    sheet.eachRow((row, rowNumber) => {
      if (rowNumber > 1) row.eachCell((cell, index) => {
  ...
  static print(purchase: Purchase, options: PurchaseFileOptions = {}) {
    ...
    const headers = hidePrice
      ? [purchaseExcelColumns[0], purchaseExcelColumns[1], purchaseExcelColumns[2], purchaseExcelColumns[6]]
      : purchaseExcelColumns;
    const rows = (purchase.lines || []).map((line) => {
      const values = purchaseLineToExcelRow(line, hidePrice);
      const visible = hidePrice ? [values[0], values[1], values[2], values[6]] : values;
      return `<tr>${visible.map((value) => `<td>${String(value ?? "")}</td>`).join("")}</tr>`;
    }).join("");
    const html = `<!doctype html><html><head><title>Phiếu nhập ${purchase.code || ""}</title><style>
    ...
    </style></head><body><h1>PHIẾU NHẬP HÀNG</h1><div class="meta"><span>Mã phiếu: ${purchase.code || ""}</span><span>Ngày: ${dayjs(purchase.orderAt).format("DD/MM/YYYY HH:mm")}</span></div><div>Nhà cung cấp: ${purchase.partner?.name || purchase.partnerSnapshot?.name || ""}</div><table><thead><tr>${headers.map((header) => `<th>${header}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table><script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()}</script></body></html>`;
