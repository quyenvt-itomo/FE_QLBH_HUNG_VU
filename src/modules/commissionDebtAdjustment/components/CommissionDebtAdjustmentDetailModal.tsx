...
export const CommissionDebtAdjustmentDetailModal: React.FC<Props> = ({ open, data, onClose, onOpenUpdate }) => {
  ...
  return (
    <Modal title={"Chi ti?t Ði?u ch?nh CN hoa h?ng"} open={open} onCancel={onClose} footer={onOpenUpdate ? <button className="text-blue-500 hover:underline text-sm" onClick={()=>onOpenUpdate(data)}>Ch?nh s?a</button> : null} width={700} destroyOnClose>
