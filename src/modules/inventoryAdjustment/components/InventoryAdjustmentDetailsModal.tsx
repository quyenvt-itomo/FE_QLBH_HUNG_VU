...
interface Props extends DetailModalProps<InventoryAdjustment> {
  onCopy?: (record: InventoryAdjustment) => void;
}

export const InventoryAdjustmentDetailsModal: React.FC<Props> = ({ open, data, onClose, onOpenUpdate, onCopy }) => {
  if (!data) return null;
  return <Modal open={open} centered destroyOnClose width={900} footer={null} title={`Chi tiết phiếu kiểm kho ${data.code || ""}`} onCancel={onClose}>
    ...
    <div className="mt-4 flex justify-end gap-2"><Button onClick={onClose}>Đóng</Button>{onCopy && <Button icon={<CopyOutlined />} onClick={() => onCopy(data)}>Sao chép</Button>}{onOpenUpdate && <Button type="primary" onClick={() => onOpenUpdate(data)}>Chỉnh sửa</Button>}</div>
  </Modal>;
};
