...
export const IncomeExpenseAddUpdateModal: React.FC<Props> = ({ open, editData, errors, loading, type, onAdd, onEdit, onClose }) => {
  ...
  const handleFinish = (values: IncomeExpense) => {
    const { category: _category, fund: _fund, partner: _partner, ...formValues } = values as IncomeExpense & Record<string, unknown>;
  ...
  return <Modal open={open} centered destroyOnClose maskClosable={false} footer={null} title={`${editData ? "Cập nhật" : "Thêm"} ${isIncome ? "phiếu thu" : "phiếu chi"}`} onCancel={handleClose} afterOpenChange={(isOpen) => {
    if (!isOpen) form.resetFields();
    else form.setFieldsValue(editData ? parseFormDataDates(editData) : ({ id, tempId: id, type, occurredAt: dayjs(), amount: 0 } as unknown as IncomeExpense));
  }}>
    <Form form={form} layout="vertical" onFinish={handleFinish} className="mt-4">
      <FormSection title={`Thông tin ${isIncome ? "phiếu thu" : "phiếu chi"}`}>
        <Row gutter={16}>
          ...
          <Col xs={24} sm={12}><Form.Item name="categoryId" label={<Label title={isIncome ? "Danh mục thu" : "Danh mục chi"} />}><AttributeManagerSelect type={isIncome ? AttributeType.INCOME_CATEGORY : AttributeType.EXPENSE_CATEGORY} defaultData={category} onChangeData={(value) => form.setFieldValue("category", value || null)} /></Form.Item><Form.Item name="category" hidden /></Col>
          <Col xs={24} sm={12}><Form.Item name="fundId" label={<Label title="Quỹ" required />} rules={[{ required: true, message: "Vui lòng chọn quỹ" }]}><FundSelect defaultData={fund} onChangeData={(value) => form.setFieldValue("fund", value || null)} /></Form.Item><Form.Item name="fund" hidden /></Col>
          ...
          <Col xs={24}><Form.Item name="partnerId" label={<Label title={isIncome ? "Khách hàng" : "Nhà cung cấp"} />}><PartnerSelect query={{ types: [isIncome ? PartnerType.CUSTOMER : PartnerType.SUPPLIER] }} defaultData={partner} onChangeData={(value) => form.setFieldValue("partner", value || null)} /></Form.Item><Form.Item name="partner" hidden /></Col>
        </Row>
      </FormSection>
      <Form.Item name="description" label={<Label title="Nội dung" />}><Input.TextArea autoSize={{ minRows: 2, maxRows: 4 }} /></Form.Item>
