...
export const IncomeExpensePage: React.FC = () => {
  ...
  const {
    ...
    setRowData,
    pageAction,
  } = usePageState<IncomeExpense>({ sortBy: "occurredAt", sortOrder: SortOrder.DESC, filterUses });
  ...
  const handleDelete = store.remove
    ? (record: IncomeExpense) =>
        modal.confirm({
          ...
          cancelText: "Hủy",
          onOk: () => store.remove?.({ id: record.id, type: record.type } as any),
        })
    : undefined;
  const handleEdit = store.update
    ...
    : undefined;
  const handleDetail = (record: IncomeExpense) => {
    setRowData(record);
    setOpenDetail(true);
  };
  const resetFilters = () => {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-3">
      <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="text-xs text-secondary">
            Theo dõi phiếu thu, phiếu chi và dòng tiền của cửa hàng
          </p>
        </div>
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setOpenType(IncomeExpenseType.INCOME);
                  setOpen(true);
                }}
              >
                + Phiếu thu
              ...
              <Button
                ...
                onClick={() => {
                  setRowData(undefined);
                  setOpenType(IncomeExpenseType.EXPENSE);
                  setOpen(true);
                }}
              >
                + Phiếu chi
      ...
      <div className="flex min-h-0 flex-1 gap-3">
        <PanelFilter
          filterActive={isFilterActive || !!filterType}
          sortItems={sortItems}
          ...
          enumFilters={[
            {
              ...
              onChange: (values: string[]) => {
                setFilterType(values[0] as IncomeExpenseType | undefined);
                setPage(1);
              },
            },
          ]}
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-lg border border-gray-200 bg-white px-4 py-3">
              <div className="text-xs text-secondary">Tổng thu trên trang</div>
              <div className="mt-1 text-lg font-semibold text-emerald-600">
                {formatMoney(totalIncome)}
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white px-4 py-3">
              <div className="text-xs text-secondary">Tổng chi trên trang</div>
              <div className="mt-1 text-lg font-semibold text-red-600">
                {formatMoney(totalExpense)}
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white px-4 py-3">
              <div className="text-xs text-secondary">Chênh lệch trên trang</div>
              <div
                className={`mt-1 text-lg font-semibold ${totalIncome - totalExpense < 0 ? "text-red-600" : "text-blue-600"}`}
              >
