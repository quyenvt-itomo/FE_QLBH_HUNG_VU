...
const FundTransferPage: React.FC = () => {
  const { keyword, page, size, setPage, setSize, pageAction } = usePageState<FundTransfer>();
  const { data, loading, pagination } = useFundTransferStore({ keyword, page, size });
    ...
    <div className="flex flex-col h-full w-full gap-3">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div className="flex flex-col">
          <h2 className="text-xl font-bold text-blue-800 dark:text-blue-200">Chuyển quỹ</h2>
          <p className="text-xs text-secondary">Quản lý chuyển quỹ</p>
        </div>
      ...
      <Panel>
        <TableColumnConfig columns={columns} dataSource={data} loading={loading} pagination={pagination} setPage={setPage} setSize={setSize} itemName="Chuyển quỹ" tableKey="fundTransfer-table" />
      </Panel>
    </div>
  );
};
