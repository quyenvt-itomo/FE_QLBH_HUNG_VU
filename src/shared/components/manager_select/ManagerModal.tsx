...
const ManagerModal = <T extends { id: string; name: string; isDefault?: boolean }>({
  ...
  const renderFormInput = () => {
    const inputProps = {
      className: `w-full ${CLASSNAME.inputHeight} border border-primary ml-2`,
      autoFocus: true,
      ...
      <div className="flex flex-col min-h-[440px]">
        <div className="flex flex-col max-h-[380px] overflow-y-auto mt-2 pb-4 overflow-x-hidden">
          {dataSource.map((item, index) => (
            ...
            >
              ...
              {editData?.id !== item.id ? (
                <>
                  <div className={`flex flex-1 ml-2 ${CLASSNAME.inputHeight} items-center`}>
                    {item.name}
                  </div>
                  {!item.isDefault && (
                    <ActionButtons
        ...
        {onAdd &&
          (isAdding ? (
            ...
            <Button
              loading={loading}
              className={`w-32 ${CLASSNAME.inputHeight} rounded-[3px] text-primary border border-primary hover:bg-primary/20 mt-2`}
