...
type GenericFilterProps<T extends { id: string }> = {
  data: T[];
...
export function GenericFilter<T extends { id: string }>({
  data,
  valueKey = "id",
  selectComponent,
  renderItem,
    ...
    <div className="card flex flex-col !gap-1 generic-filter">
      ...
      {data.map((item) => (
        <div
          ...
          onClick={() => onRemove?.(String(item[valueKey]))}
        >
          {renderItem ? renderItem(item) : <span>{String(item[valueKey])}</span>}
