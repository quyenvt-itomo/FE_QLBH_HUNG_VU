...
export const parseDynamicParams = (
  ...
  try {
    ...
    while ((match = pattern.exec(url)) !== null) {
      const field = match[1]; // vd: productId
      if (!payload || typeof payload[field] !== "string") {
        return {
          ...
          errorResponse: {
            ...
            menu: [],
            detailError: [{ field, message: `${field}.missing` }],
          },
        };
      }

      // thay vào url
      newUrl = newUrl.replace(`:${field}`, payload[field]);
      delete payload[field]; // xoá khỏi body/params để tránh bị gửi đi
    }
    return { url: newUrl };
  } catch (error) {
    ...
    return {
      ...
      errorResponse: {
        ...
        menu: [],
        detailError: [{ field: "url", message: "url.parsing.error" }],
      },
    };
  }
};
...
// Hàm POST trả về Blob (dùng cho download file binary)
export const postBlob = async (url: string, data: any): Promise<Blob> => {
  try {
