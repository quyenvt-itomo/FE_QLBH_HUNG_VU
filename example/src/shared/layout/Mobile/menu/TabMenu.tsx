...
const MobileTabMenu: React.FC = () => {
  ...
  const isActivePathCategories = () =>
    otherItems.some((child: any) => location.pathname.startsWith(child.key.toString()));
  ...
  return (
    <div className="z-50 bg-panel border-t border-gray-200 shadow-md md:hidden">
          ...
          <Dropdown menu={setupMenu} placement="topRight" trigger={["click"]}>
            <div
              className={`flex-1 flex flex-col items-center text-xs z-10 ${
                isActivePathCategories() ? "text-white font-normal" : "text-gray-500"
              } cursor-pointer`}
            >
