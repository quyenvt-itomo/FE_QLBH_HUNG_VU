...
export const AddUpdateBillOfMaterialModal: React.FC<AddUpdateModalProps<BillOfMaterial>> = ({
  ...
  return (
    <Modal
      title={editData ? "Sửa Định mức NVL" : "Thêm Định mức NVL"}
      open={open}
      onCancel={onClose}
      footer={null}
      width={1000}
    ...
    >
      ...
      >
        ...
        <Form.Item
          name="productId"
          label="Hàng hóa (thành phẩm)"
          rules={[{ required: true, message: "Vui lòng chọn hàng hóa" }]}
        >
          <ProductSelect
        ...
        <Title content="Công đoạn & Nguyên vật liệu" />
              ...
              {opFields.map(({ key, name, ...restField }) => {
                ...
                return (
                  <div key={key} className="border rounded p-3 bg-gray-50/70 flex flex-col gap-2">
                    ...
                    {/* FormList lồng: materials của công đoạn này */}
                    <Form.List name={[name, "materials"]}>
                      {(matFields, { add: addMaterial, remove: removeMaterial }) => (
                        <div className="ml-4 border-l pl-3 flex flex-col gap-2">
                          <div className="text-xs font-semibold text-gray-500">Nguyên vật liệu</div>
