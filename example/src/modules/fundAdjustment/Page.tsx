...
const FundAdjustmentPage: React.FC = () => {
  const { keyword, page, size, setPage, setSize, pageAction } = usePageState<FundAdjustment>();
  const { data, loading, pagination } = useFundAdjustmentStore({ keyword, page, size });
    ...
    <div className="flex flex-col h-full w-full gap-3">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div className="flex flex-col">
          <h2 className="text-xl font-bold text-blue-800 dark:text-blue-200">Điều chỉnh quỹ</h2>
          <p className="text-xs text-secondary">Quản lý điều chỉnh quỹ</p>
        </div>
      ...
      <Panel>
        <TableColumnConfig columns={columns} dataSource={data} loading={loading} pagination={pagination} setPage={setPage} setSize={setSize} itemName="Điều chỉnh quỹ" tableKey="fundAdjustment-table" />
      </Panel>
    </div>
  );
};
