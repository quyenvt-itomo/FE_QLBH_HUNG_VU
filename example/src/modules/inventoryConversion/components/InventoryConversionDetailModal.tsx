...
export const InventoryConversionDetailModal: React.FC<Props> = ({ open, data, onClose, onOpenUpdate }) => {
  ...
  return (
    <Modal title={"Chi tiết Chuyển mã"} open={open} onCancel={onClose} footer={onOpenUpdate ? <button className="text-blue-500 hover:underline text-sm" onClick={()=>onOpenUpdate(data)}>Chỉnh sửa</button> : null} width={700} destroyOnClose>
