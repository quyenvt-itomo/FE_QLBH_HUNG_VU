...
export const AddUpdateInventoryConversionModal: React.FC<
  AddUpdateModalProps<InventoryConversion>
> = ({ open, editData, loading, errors, onAdd, onEdit, onClose }) => {
  const [form] = Form.useForm<InventoryConversion>();
  ...
  useEffect(() => {
    ...
    if (editData) form.setFieldsValue(editData);
      ...
      form.resetFields();
      setFormCode({ form, type: "inventoryConversion" });
    }
  }, [open, editData, form]);
  return (
    <Modal
      title={editData ? "Sửa Chuyển mã" : "Thêm Chuyển mã"}
      open={open}
      onCancel={onClose}
      footer={null}
      width={640}
