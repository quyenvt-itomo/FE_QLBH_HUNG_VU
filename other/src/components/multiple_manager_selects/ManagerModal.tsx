...
import { MultipleManagerModalProps } from "../../models/base/interface";
...
const MultipleManagerModal = <T extends { id: string; name: string; isDefault?: boolean }>({
  ...
  const renderFormInput = () => {
    const inputProps = {
      autoFocus: true,
      className: "ml-2 w-full h-8 border border-primary",
  ...
  return (
    ...
    >
      <div className="flex flex-col min-h-[440px]">
        <div className="flex flex-col max-h-[350px] overflow-y-auto mt-2 pb-4 overflow-x-hidden">
          {dataSource.map((item) => (
            <div
              key={item.id}
