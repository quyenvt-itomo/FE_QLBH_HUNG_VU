...
export const OrderDiscountInput: React.FC<Props> = ({
  ...
  return (
    <div
      className={`flex w-full items-center overflow-hidden rounded transition h-8 ${
        borderNone ? "h-8" : "border border-gray-300 hover:border-primary"
      }`}
    >
      ...
      <Select
        ...
        variant="borderless"
        suffixIcon={null}
        className={`!w-9 shrink-0 h-8 !border-none !shadow-none`}
      />
    </div>
  );
};
