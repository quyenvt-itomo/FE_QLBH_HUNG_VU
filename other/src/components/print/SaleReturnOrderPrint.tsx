...
const SaleReturnOrderPrint: React.FC<Props> = ({ data, qrImage }) => {
  const returnLines =
  ...
  const renderLine = (item: IOrder["lines"][0], isReturn: boolean) => (
    <tr key={item.id}>
      ...
      <td className="text-right border-b border-dashed border-[#323832] px-1 py-0.5">
        {isReturn ? `-${formatQuantity(Math.abs(item.quantity))}` : formatQuantity(item.quantity)}
      </td>
      <td className="text-right border-b border-dashed border-[#323832] px-1 py-0.5">
        {isReturn
          ? `-${formatMoney(Math.abs(item.subTotal - (item.discountAmount || 0)))}`
      ...
      <div className="text-[9px] leading-[1.4] w-[57mm] max-w-[57mm] overflow-x-hidden p-[2mm] [&_*]:text-[9px] [&_*]:leading-[1.4]">
        ...
        <main className="mt-2">
          <table className="w-full border-collapse table-fixed mt-2">
            ...
            <tbody>
              {normalLines.map((item) => renderLine(item, false))}
              {returnLines.length > 0 && (
                <tr>
                  <td
                    colSpan={3}
                    className="text-center font-semibold pt-2 border-t-2 border-[#323832]"
                  >
                    HÀNG HOÀN TRẢ
                  </td>
                </tr>
              )}
              {returnLines.map((item) => renderLine(item, true))}
            </tbody>
          </table>
        </main>
...
export default SaleReturnOrderPrint;
