...
const AddressBank: React.FC<AddressBankProps> = ({ customer, onEdit }) => {
  ...
  const handleDelete = (type: UpdateType, item: { index: number }) => {
    if (type === "address") {
      const newAddresses = customer.addresses.filter((_, idx) => idx !== item.index);

      onEdit?.({ addresses: newAddresses, id: customer.id });
    }

    if (type === "bank") {
      const newBanks = customer.banks.filter((_, idx) => idx !== item.index);
    ...
    <div className="flex gap-36 w-full">
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col gap-6 w-full mt-4">
          {customer.addresses.map((i, index) => (
            <AddressCardBase
              key={`address-${index}`}
              item={i}
