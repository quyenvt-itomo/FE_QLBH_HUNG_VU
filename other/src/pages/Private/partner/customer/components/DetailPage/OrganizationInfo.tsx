...
import { useCustomerContactData } from "../../../../../../hooks/partner/useCustomerContactData";
...
export const OrganizationInfo: React.FC<OrganizationInfoProps> = ({
  ...
  const { addCustomerContact, updateCustomerContact, deleteCustomerContact } =
  ...
  return (
    <div className="shadow-md bg-white rounded-2xl overflow-hidden slide-left border border-gray-100 h-[calc(100vh-160px)] overflow-y-auto scrollbar-hide mb-4">
      <div className="p-4 h-full">
        ...
        <Row className="px-8 py-6" gutter={[108, 28]}>
          <Col lg={12} xs={24}>
            ...
            <EditableInfoRow<IPartner>
              label="Chức vụ"
              value={customer?.representative?.position}
          ...
          <Col lg={12} xs={24}>
            <EditableInfoRow<IPartner>
              label="Email"
              value={customer?.representative?.email}
            ...
            <EditableInfoRow<IPartner>
              label="Số điện thoại"
              value={customer?.representative?.phone}
            ...
            <ActionButtonsNoHover
              ...
              onDelete={
                deleteCustomerContact
                  ? () => {
                      deleteCustomerContact(contact.id);
                    }
                  : undefined
              }
            />
      ...
      {addCustomerContact && updateCustomerContact && (
        <AddUpdateContactModal
          ...
          onAdd={addCustomerContact}
          onEdit={updateCustomerContact}
        />
      )}
    </div>
  );
