...
import { useSupplierSubTypeData } from "../../../../../../hooks/partner/useSupplierSubTypeData";
...
export const CardInfo: React.FC<CardInfoProps> = ({ supplier, onUpdate, onReload }) => {
  const [subTypes, setSubTypes] = useState<IPartnerSubType[]>([]);

  const { canUpdate, addSubType, updateSubType, deleteSubType } = useSupplierSubTypeData({
  ...
  // Sync subTypes from supplier data
  useEffect(() => {
    if (!supplier?.subTypes) return;
    setSubTypes(supplier.subTypes);
  }, [supplier?.subTypes]);
  ...
  const handleUpdateSubType = async (type: PartnerTypeEnum, groupId: string) => {
    const existing = supplier.subTypes.find((i) => i.type === type);
          ...
          <div className="flex flex-col items-center p-8 pt-4">
            <div className="relative">
              <AvatarUpload
                ...
                oId={supplier.id}
                defaultFile={getMainImage(supplier.avatar)}
                isActive
              />
            </div>
            ...
            <EditableHeaderText<IPartner>
              ...
              displayComponent={<p className="text-white/90 text-center">{supplier.code}</p>}
            />
                ...
                content={
                  <div className="w-80 p-1">
                    ...
                    <AttributeSelect
                      ...
                      placeholder=""
                      value={supplier.groupId || undefined}
                      defaultData={supplier.group}
                      className="mt-2"
                      onChange={(value) => {
                        onUpdate?.({
                          groupId: value,
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3 border-b border-gray-100">
              <EditSubTypeRow
                ...
                value={subTypes?.find((i) => i.type === PartnerTypeEnum.CUSTOMER)}
                attributeType={AttributeTypeEnum.CUSTOMER_GROUP}
                disabled={!canUpdate}
                onAdd={(groupId) => handleAddSubType(PartnerTypeEnum.CUSTOMER, groupId)}
                onUpdate={(groupId) => handleUpdateSubType(PartnerTypeEnum.CUSTOMER, groupId)}
                onDelete={() => handleDeleteSubType(PartnerTypeEnum.CUSTOMER)}
              />
            </div>
            <div className="flex items-center gap-3 border-b border-gray-100">
              <EditSubTypeRow
