...
const AddPage: React.FC = () => {
  ...
  const { loading, errors, addSupplier } = useSupplierData({
    isLockHook: true,
    onCloseModal: () => {
      navigate(privateRoutesName.supplier.page, { replace: true });
    },
  });
        ...
        >
          ...
          <div className="w-full flex flex-col bg-white rounded-lg px-12 gap-4 overflow-y-auto overflow-x-hidden">
            <div className="sticky top-0 z-10 bg-white border-b py-4 flex justify-between items-center">
              <Title content="Thêm nhà cung cấp" level={2} />
              <SubmitButton
                onCancel={() => {
                  navigate(privateRoutesName.supplier.page);
                }}
                loading={loading}
              />
            </div>
