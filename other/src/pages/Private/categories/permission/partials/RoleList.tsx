...
interface RoleListProps {
  ...
  setSelectedRow: (record: IRole) => void;
  onAdd?: (record: IRole) => void;
...
const RoleList: React.FC<RoleListProps> = ({
  ...
  const [deleteData, setDeleteData] = useState<IRole | null>(null);
  ...
  return (
    <div className="flex flex-col relative w-full h-full px-3">
      {loading && (
      ...
      {!currentStore && (
        <StoreSelect
          value={storeId}
          onChange={(value) => setStoreId && setStoreId(value)}
      ...
      <div className="flex flex-col flex-1 gap-2 overflow-x-hidden overflow-y-auto mt-4">
        ...
        {!onAdd ? (
          ...
          <Form form={addForm} onFinish={handleAdd} className="flex">
            <Form.Item
              name="name"
              className="flex-1"
              rules={[
