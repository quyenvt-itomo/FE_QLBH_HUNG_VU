...
interface SystemRolePermissionProps {
  loading: boolean;
  selectedRow: ISystemRole | null;
  onUpdateSystemRolePermission?: (record: Partial<ISystemRole>) => void;
  onToggleDrawer: () => void;
}

const SystemRolePermission: React.FC<SystemRolePermissionProps> = ({
  loading,
  selectedRow,
  onUpdateSystemRolePermission,
  onToggleDrawer,
}) => {
  ...
  return (
    <Form form={form} onFinish={handleFinish} className="flex flex-col h-full relative p-3">
      <div className="flex items-center gap-3 justify-between h-8">
        ...
        {onUpdateSystemRolePermission ? (
          <SubmitButton loading={loading} onCancel={handleReset} />
        ) : (
          <></>
        )}
      </div>
                ...
                {group.modules.map((module) => {
                  return (
                    <PermissionSelect
                      ...
                      form={form}
                      module={module}
                      disabled={!onUpdateSystemRolePermission}
                    />
                  );
                })}
              </div>
...
export default SystemRolePermission;
