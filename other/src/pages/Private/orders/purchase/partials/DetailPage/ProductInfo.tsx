...
import { usePurchaseLineData } from "../../../../../../hooks/order/usePurchaseLineData";
import SquareImage from "../../../../../../components/image/SquareImage";
import { getMainImage } from "../../../../../../utils/fileUtil";
import { getProductVariantByBarcode, sortData } from "../../../../../../utils/common";
...
const ProductInfo: React.FC<PartialProps> = ({ data, itemForm, onReload, onUpdate }) => {
  ...
  const totalQuantity = items.reduce((total, line) => total + (line.quantity || 0), 0);
  ...
  const { addPurchaseLine, updatePurchaseLine, deletePurchaseLine } = usePurchaseLineData({
    orderId: data?.id,
    onCloseModal: () => {
      onReload?.();
      handleClose();
  ...
  const handleOpenAddLine = onUpdate
    ? (variant: IProductVariant) => {
        const existingIndex = data.lines.findIndex((line) => line.productVariantId === variant.id);
        if (existingIndex !== -1) {
          message.warning(
            "Sản phẩm đã có trong đơn hàng, vui lòng chỉnh sửa số lượng nếu muốn thêm.",
          );
          return;
        }
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-[calc(100%-80px)]">
      ...
      <div ref={containerRef} className="flex flex-col flex-1 border rounded-md overflow-auto z-0">
