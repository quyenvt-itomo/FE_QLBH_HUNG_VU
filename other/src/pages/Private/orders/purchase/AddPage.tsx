...
const AddPage: React.FC = () => {
  ...
  const { newPurchase, errors, loading, addPurchase } = usePurchaseData({
    isLockHook: true,
    onCloseModal: () => {},
  });
  ...
  const handleFinish = async (values: IOrder) => {
    try {
      ...
      addPurchase?.(formattedData);
    } catch (error) {
      console.error("Error adding purchase:", error);
    }
  };
  useEffect(() => {
    setFormCode({ form, type: "purchase", field: "code" });
  }, []);
  ...
  useEffect(() => {
    ...
    const url = buildUrlWithId(privateRoutesName.purchase.detail, newPurchase.id);
