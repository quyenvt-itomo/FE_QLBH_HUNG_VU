...
const InvoiceInfo: React.FC<PartialProps> = ({ form, loading }) => {
  ...
  function calculateFee() {
    const result = {
      totalMoney: 0, // tiền hàng gốc (normal - return)
      totalProductDiscount: 0, // giảm giá SP
      totalOrderDiscount: 0, // giảm giá đơn (chỉ phân bổ cho normal)
      totalTaxableAmount: 0, // tiền tính thuế (sau mọi giảm)
      totalVat: 0, // tổng VAT (normal - return)
      totalAmount: 0, // tổng phải trả
    };

    // Tách lines thành returnLines và normalLines
    const returnLines = lines.filter((line) => line.lineType === OrderLineTypeEnum.RETURN);
    const normalLines = lines.filter(
      (line) => !line.lineType || line.lineType === OrderLineTypeEnum.NORMAL,
    );

    // =========== XỬ LÝ RETURN LINES (trừ vào đơn) ===========
    returnLines.forEach((item) => {
      ...
      // Trừ vào tổng (return lines giảm giá trị đơn)
      result.totalMoney -= money;
      result.totalProductDiscount -= productDiscount;
      result.totalTaxableAmount -= baseAmount;
      result.totalVat -= vatAmount;
    });

    // =========== XỬ LÝ NORMAL LINES (cộng vào đơn) ===========
    const normalItems: {
      baseAmount: number;
      vatRate: number;
    }[] = [];

    // 1️⃣ Tính tổng tiền và giảm giá SP cho normal lines
    ...
    // 2️⃣ Tổng tiền của normal lines sau giảm SP (dùng để phân bổ giảm đơn)
    const totalNormalBaseAmount = normalItems.reduce((sum, i) => sum + i.baseAmount, 0);

    // 3️⃣ Tính giảm giá đơn (chỉ áp dụng cho normal lines)
    const orderDiscountAmount =
      discountType === DiscountTypeEnum.PERCENT
        ? (totalNormalBaseAmount * orderDiscount) / 100
        : orderDiscount || 0;

    result.totalOrderDiscount = Math.min(orderDiscountAmount, totalNormalBaseAmount);

    // 4️⃣ Phân bổ giảm giá đơn + tính VAT cho normal lines
    let allocatedSum = 0;
    ...
    normalItems.forEach((item, index) => {
      ...
      if (normalItems.length > 0) {
        if (index === normalItems.length - 1) {
          ...
          allocatedDiscount =
            (item.baseAmount / totalNormalBaseAmount) * result.totalOrderDiscount;
  ...
  return (
    <div className="flex flex-col bg-white rounded-lg p-4 pb-0 flex-1">
      ...
      <div className="flex flex-col mt-6">
        <div className="relative">
          <Form.Item name={["payment", "amount"]} label={<Label title="Hoàn tiền KH" />}>
