...
import { resetCheckResetPassword, resetPassword } from "../../../stores/auth/slice";
...
const ConfirmPasswordPage: React.FC<PageProps> = () => {
  ...
  const onFinish: FormProps["onFinish"] = (values: {
    ...
    const { password, confirmPassword, isLogout, storeCode, storeName } = values;
    ...
    dispatch(resetPassword({ password, isLogout }));
  };

  const { isCheckResetPassword, loading } = useSelector(
    (state: RootState) => state.Auth,
    shallowEqual,
  );

  useEffect(() => {
    if (!isCheckResetPassword) return;

    dispatch(resetCheckResetPassword());

    setTimeout(() => {
      navigate(publicRoutesName.login);
    }, 1000);
  }, [isCheckResetPassword, form]);
